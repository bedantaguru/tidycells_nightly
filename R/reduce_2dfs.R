...
reduce_2dfs <- function(dc1, dc2, combine_th = 0.6, rest_cols = Inf, retain_other_cols = FALSE){
  ...
  m1 <- 1:nrow(all_maps) %>% map(~similarity_score(cr1[[all_maps$n1[.x]]], cr2[[all_maps$n2[.x]]]) %>% t()) %>% reduce(rbind)
  m2 <- as.data.frame(m1) %>% map(norm_this) %>% as.data.frame()
  # KFL : kept for later
  wts <- rep(1, ncol(m2))
  ...
  if(!retain_other_cols){
    nc_cols <- colnames(dcnew) %>% stringr::str_detect("collated") %>% colnames(dcnew)[.]
